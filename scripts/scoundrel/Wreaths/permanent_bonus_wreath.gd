...
func _init(art : String, bonus: int, validSuitTargets : Array[TDCardData_Art.SuitType], price : int, wreathName : String = "PermanentBonusWreath"):
	super._init(art, price, wreathName)
	ValidSuitTargets = validSuitTargets
	Bonus = bonus
	return

func Copy() -> Wreath:
	return PermanentBonusWreath.new(Art.resource_path, Bonus, ValidSuitTargets, Price, WreathName)
